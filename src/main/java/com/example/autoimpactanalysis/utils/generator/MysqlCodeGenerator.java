...
public class MysqlCodeGenerator {
    ...
    private static void generator(){
        FastAutoGenerator.create("jdbc:mysql://127.0.0.1:3306/developdatabase?serverTimezone=GMT%2b8", "root", "123456")
                .globalConfig(builder -> {
                    builder.author("kevin") // 设置作者
                            .enableSwagger() // 开启 swagger 模式
                            .fileOverride() // 覆盖已生成文件
                            .outputDir("D:\\kevin\\workspace\\work_workspace\\autoimpactanalysis\\src\\main\\java\\")// 指定输出目录
                            .dateType(DateType.ONLY_DATE);
                })
                .packageConfig(builder -> {
                    builder.parent("com.example.autoimpactanalysis") // 设置父包名
                            .moduleName(null) // 设置父包模块名 若为""则多一个/
                            .pathInfo(Collections.singletonMap(OutputFile.mapperXml, "D:\\kevin\\workspace\\work_workspace\\autoimpactanalysis\\src\\main\\resources\\mapper\\")); // 设置mapperXml生成路径
                })
