<template>
    <div>
        ...
        <div style="padding-bottom: 10px; padding-left: 20px;">
            ...
            <el-input style="width: 200px" placeholder="请输入问题关键字" suffix-icon="el-icon-message" class="margin-5"
                      v-model="busConfirmQuestion"></el-input>
            <el-input style="width: 200px" placeholder="请输入回答关键字" suffix-icon="el-icon-message" class="margin-5"
                      v-model="busConfirmReply"></el-input>
            ...
            <el-button class="margin-5" type="primary" @click="exportBatchClick">批量导出 <i class="el-icon-download"></i>
            </el-button>
        </div>
        <el-table :data="tableData" border stripe class="tableClass"
            ...
            <el-table-column prop="busConfirmState" label="业务确认状态" width="150" :formatter="busConfirmStateFormat"
                             :filters="busConfirmStateOptions" :filter-method="filterBusConfirmState" sortable>
            </el-table-column>
            <el-table-column prop="busConfirmType" label="业务确认类型" width="150" :formatter="busConfirmTypeFormat"
                             :filters="busConfirmTypeOptions" :filter-method="filterBusConfirmType" sortable>
            </el-table-column>
            ...
            <el-table-column label="操作" width="380">
                <template slot-scope="scope">
                    ...
                    <el-popconfirm
                            ...
                            icon-color="red"
                            title="定稿之后不可修改！"
                            @confirm="editConfirmState(scope.row)">
            ...
            <el-form label_witch="100px" size="small" :model="form" :rules="rules" ref="busConfirmForm">
                <el-form-item label="项目名称" prop="projectId">
                    <el-select v-model="form.projectId" placeholder="请选择项目名称" filterable default-first-option
                               style="width: 600px;" @change="getReportList(form.projectId)">
                        <el-option
                ...
                <el-form-item label="业务确认类型" prop="busConfirmType">
                    <el-select v-model="form.busConfirmType">
                        <el-option
                                v-for="b in busConfirmTypeOptions"
                                :key="b.busConfirmType"
                                :label="b.label"
                                :value="b.busConfirmType">
                        </el-option>
                    </el-select>
                </el-form-item>
        ...
        <el-dialog title="业务确认查看" :visible.sync="busConfirmDialogView" width="70%" top="10px" style="margin-top: 0px"
            ...
            <el-form label_witch="100px" size="small">
                ...
                <div class="inputCLass">
                    业务确认状态:
                    <el-select v-model="form.busConfirmState" class="noBorderSelect" disabled
        ...
        methods: {
            ...
            getReportList(projectId) {
                axios.get("/reportDetail/getByProjectId/" + projectId).then(data => {
                    console.log("getReportList:" + projectId);
                    console.log(data);
                ...
                axios.get(
                    "/busConfirm/findPage", {
                        params: {
                            ...
                            busConfirmQuestion: this.busConfirmQuestion,
                            busConfirmReply: this.busConfirmReply
                        }
                    }
                ).then(data => {
            ...
            handleEdit(row) {
                if ("1" !== row.busConfirmState) {
                    ...
                    this.reportOptions = [];
                    this.getProjectList();
                    this.getReportList(row.projectId);
                    this.form = row;
                    this.form.busConfirmTime = Date.parse(new Date(row.busConfirmTime));
                    this.initSuccess = true;
            ...
            deleteRow(row) {
                if ("1" !== row.busConfirmState) {
                    let userVO = JSON.parse(localStorage.getItem("user"));
                    this.form.operator = this.form.operator + ", " + userVO.userName;
                    axios.get("/busConfirm/delete/", {
                        params: {
                            busConfirmId: row.busConfirmId,
            ...
            handleSelectionChange(val) {
                ...
                for (let i = 0; i < val.length; i++) {
                    ...
                    let bus = {};
                    bus["busConfirmId"] = val[i].busConfirmId;
                    bus["operator"] = val[i].operator + ", " + userVO.userName;
                    this.deleteSelection[i] = bus;
                }
            },
            // 批量删除
            deleteBatch() {
                ...
                if (flag === false) {
                    ...
                    let busConfirms = this.deleteSelection;
                    axios.post("/busConfirm/deleteBatch/", busConfirms).then(data => {
                        if (data) {
            ...
            exportBatchClick() {
                if (this.downloadSelection.length < 1) {
                    ...
                    // 用 FielSaver.js里的保存方法 进行输出
                    saveAs(converted, '问题确认批量导出.docx');
                }
